...
if (!defined($session)) {
        ...
        switch ($o_option) {
                ...
                case "va_rating"                { va_rating(); }
                case "battery_remaining_time"   { battery_remaining_time(); }
                case "battery_level"            { battery_level(); }
                case "battery_replacement"      { battery_replacement(); }
                case "battery_fault"            { battery_fault(); }
                case "battery_voltage"          { battery_voltage(); }
                case "battery_low_battery"      { battery_low_battery(); }
                case "battery_charger_fault"    { battery_charger_fault(); }
                case "battery_low_condition"    { battery_low_condition(); }
                case "input_voltage"            { input_voltage(); }
                case "input_frequency"          { input_frequency(); }
                case "input_line_fail_cause"    { input_line_fail_cause(); }
                case "input_bad_status"         { input_bad_status(); }
                case "output_load"              { output_load(); }
                case "output_voltage"           { output_voltage(); }
                case "output_frequency"         { output_frequency(); }
                case "output_on_battery"        { output_on_battery(); }
                case "output_on_by_pass"        { output_on_by_pass(); }
                case "output_over_load"         { output_over_load(); }
                case "output_over_temp"         { output_over_temp(); }
                case "output_utility_off"       { output_utility_off(); }
                case "output_inverter_off"      { output_inverter_off(); }
                case "ambient_temperature"      { ambient_temperature(); }
                case "ambient_humidity"         { ambient_humidity(); }
                case "agent_firmware_version"   { agent_firmware_version(); }
                case "agent_mib_version"        { agent_mib_version(); }
...
sub input_line_fail_cause_test {
    ...
    verb("upsmgInputLineFailCause OID response : $upsmgInputLineFailCause");
        ...
        switch ($upsmgInputLineFailCause) {
            case "1"    { $returnstring = "OK"; }
            case "2"    { $returnstring = "Voltage out of tolerance"; $status = 1; }
            case "3"    { $returnstring = "Frequency out of tolerance"; $status = 1; }
            case "4"    { $returnstring = "No voltage at all"; $status = 1; }
        }
}
