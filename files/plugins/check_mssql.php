...
# Version 0.6.7,0.6.8          2012 Nicholas Scott <nscott@nagios.com>
# Notes:
#
...
if (isset($critical)) {
        if (!preg_match("/^[0-9]\d*(\.\d+)?$/", $critical)) {
...
process_results($query_duration, $warning, $critical, $output_msg, $exit_code);
...
function process_results($query_duration, $warning, $critical, $output_msg, $exit_code = null) {
    if ($query_duration > $critical || $exit_code == 2) {
                $state = "CRITICAL";
                $exit_code = 2;
        } elseif ($query_duration > $warning || $exit_code == 1) {
                $state = "WARNING";
                $exit_code = 1;
        } else {
                $state = "OK";
    ...
    $perfdata = "query_duration={$query_duration}s;{$warning};{$critical};0;";
