...
if (count($resultSet)) {
	...
	*/
	...
	if ($failed_jobs > 0) {
		$return_code = 2;
		$return_msg = $failed_jobs . " jobs failed, $succeeded_jobs jobs OK\n";
		foreach ( $failed_jobArray as $failed_job ) {
			$return_msg = $return_msg . "Job-Name: " . $failed_job['job_name'] . "\tJob-Status: " . $failed_job['last_run_status'] . "\tDescription: " . $failed_job['step_description'] . "\tLast Run: " . $failed_job['last_run_date'] . "\n";
		}
	} else {
		$return_msg = "All jobs are OK, $succeeded_jobs jobs OK";
		$return_code = 0;
	}
} else {
	$return_msg = "No jobs found";
	$return_code = 0;
}
