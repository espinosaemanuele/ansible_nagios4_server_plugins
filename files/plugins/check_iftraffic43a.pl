...
if ($in_traffic<$min_value) {
 stop("CRITICAL Traffic IN for $iface_descr - $in_traffic$suffix, No Traffic Throughput! Please check GGSN Tunnel, for Interface: $iface_descr\n|$iface_descr inUsage=$in_usage%;$warn_usage;$crit_usage outUsage=$out_usage%;$warn_usage;$crit_usage inBandwidth=$in_traffic$suffix outBandwidth=$out_traffic$suffix", "CRITICAL");
	$state = "CRITICAL";
}
...
if (($in_usage>$warn_usage) or ($out_usage>$warn_usage) or (($in_usage>$warn_usage) && ($out_usage>$warn_usage))) {
 stop("WARNING Utilization Alarm for $iface_descr - Average IN: $in$suffix $in_usage% Average OUT: $out$suffix $out_usage%\n|$iface_descr inUsage=$in_usage%;$warn_usage;$crit_usage outUsage=$out_usage%;$warn_usage;$crit_usage inBandwidth=$in_traffic$suffix outBandwidth=$out_traffic$suffix", "WARNING");
        $state = "WARNING";
}
...
if ( $state ne "UNKNOWN" ){
 stop("OK for $iface_descr - Average IN: $in$suffix $in_usage% Average OUT: $out$suffix $out_usage%\n|$iface_descr inUsage=$in_usage%;$warn_usage;$crit_usage outUsage=$out_usage%;$warn_usage;$crit_usage inBandwidth=$in_traffic$suffix outBandwidth=$out_traffic$suffix", "OK");
	$state = "OK";
}
